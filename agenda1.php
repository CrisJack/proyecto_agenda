...
<div class="row">
    <h1>
            <?php
            if (isset ($_SESSION['iniciosesion'])) {
            echo "Bienvenido(a): ".$_SESSION['iniciosesion']."<br>Tu agenda de esta semana es:";
            }else{
...
<!-- Modal -->
  ...
  //  $Mes_Anyo = strftime("%A, %d de %B de %Y", strtotime($Nueva_Fecha));
   ...
   ?>
    ...
    ?>  
    <tr>  
      <th> <?php echo $valor['hora'];?> </th>
      <?php     
   for($i=0;$i<7;$i++){
       $Nueva_Fecha= date("Y-m-d",strtotime($fecha_actual."+ $i days"));
       $Mes_Anyo = strftime("%A, %d", strtotime($Nueva_Fecha)); 

       if ($valor['fecha']==$Nueva_Fecha){
        ?>
        <td>
        <form action="modificar_evento.php" method="POST">
        <button type="submit" name="id" style="border:none; background:none;"  value="<?php echo $valor['id_evento'];?>"><?php echo $valor['titulo'];?></button>          
         </form>
         </td>
        <?php 
       }else{
        ?>
        <td><span style="color:red;"> <?php echo "sin agenda";?> </span></td>
        <?php 
       }
